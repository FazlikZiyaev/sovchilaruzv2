...
import { EntityArrayResponseType, ContactInfoService } from '../service/contact-info.service';
...
export class ContactInfoComponent implements OnInit {
  ...
  trackId = (_index: number, item: IContactInfo): string => this.contactInfoService.getContactInfoIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.contactInfos = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IContactInfo[] | null): IContactInfo[] {
    // If there is previus link, data is a infinite scroll pagination content.
    if ('prev' in this.links) {
      const contactInfosNew = this.contactInfos ?? [];
      if (data) {
        for (const d of data) {
          if (contactInfosNew.map(op => op.id).indexOf(d.id) === -1) {
            contactInfosNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.contactInfoService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
